...
| target           | load s   | nodes/s   | edges/s   | nodes / edges held   | indexed   | status                                                                                 |
|------------------|----------|-----------|-----------|----------------------|-----------|----------------------------------------------------------------------------------------|
| arangodb         | 7.7      | 3,607     | 45,820    | 27,770 / 352,768     | yes       | verified                                                                               |
| aura-free        | 32.7     | 849       | 10,785    | 27,770 / 352,768     | yes       | verified                                                                               |
| cognodb-cloud    | -        | -         | -         | -                    | -         | not reachable - cognodb-cloud: could not connect to bolt+s://<cognodb-endpoint-redacted> |
| memgraph         | 7.0      | 3,972     | 50,452    | 27,770 / 352,768     | yes       | verified                                                                               |
| neo4j-selfhosted | 28.6     | 971       | 12,329    | 27,770 / 352,768     | yes       | verified                                                                               |

## Resources and footprint

| target           | vCPU   | memory   | limit is                                       | runtime footprint   |
|------------------|--------|----------|------------------------------------------------|---------------------|
| arangodb         | 1      | 2 GB     | enforced by this repo (`make probe` verifies)  | not observable      |
| aura-free        | 1      | 1 GB     | vendor-advertised, not verifiable from outside | not observable      |
| cognodb-cloud    | 1      | 1 GB     | vendor-advertised, not verifiable from outside | not observable      |
| memgraph         | 1      | 2 GB     | enforced by this repo (`make probe` verifies)  | not observable      |
| neo4j-selfhosted | 1      | 2 GB     | enforced by this repo (`make probe` verifies)  | not observable      |

**`runtime footprint` is `not observable` for every target, and that is a statement about this harness rather than about the engines.** The benchmark is a client: it can report the limits it applied and the counts a server will tell it, but resident memory and on-disk size are visible only from inside the host. For the containers those figures could be obtained with `docker stats`; for the managed tiers they cannot be obtained at all. Rather than publish one column measured and another estimated, neither is claimed.

The self-hosted caps are applied by `infra/docker-compose.yml` and confirmed inside the container by `scripts/probe_limits.py`, which reads the cgroup files directly - a nested Docker daemon can accept `--memory` and silently ignore it, and a parity claim resting on an unapplied cap would be worthless.

The managed tiers are the vendor's to define. Their figures are recorded as advertised and are **not** independently verified, which is the main reason a managed-versus-container latency gap should be read through the Aura-versus-self-hosted-Neo4j anchor rather than taken at face value.
...
**No results at all: falkordb.** These targets are configured but produced no usable result file, so they appear in no table above. Their absence is a gap in this benchmark, not a judgement about them: nothing here says whether they would have been faster or slower. Any comparison drawn from this report covers only the targets that actually ran.

- **falkordb**: Not disabled. A 100-iteration run was in progress when the Codespace restarted and the process was killed before writing a result file. The earlier smoke run reached it successfully, so this is an interrupted measurement rather than an engine that could not be benchmarked. Nothing in this report says whether it would have been faster or slower.
